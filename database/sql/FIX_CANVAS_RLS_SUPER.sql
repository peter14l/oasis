-- =====================================================
-- CANVAS & CIRCLE RLS - SUPER FIX (RECURSION-FREE)
-- =====================================================
...
RETURNS boolean LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM canvas_members WHERE canvas_id = c_id AND user_id = auth.uid());
$$;
...
RETURNS boolean LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (SELECT 1 FROM circle_members WHERE circle_id = c_id AND user_id = auth.uid());
$$;
...
CREATE POLICY "Select_Canvases" ON canvases FOR SELECT USING ( public.is_canvas_member(id) OR created_by = auth.uid() );
CREATE POLICY "Insert_Canvases" ON canvases FOR INSERT WITH CHECK ( auth.uid() = created_by );
CREATE POLICY "Update_Canvases" ON canvases FOR UPDATE USING ( public.is_canvas_member(id) );
CREATE POLICY "Delete_Canvases" ON canvases FOR DELETE USING ( public.is_canvas_owner(id) );

-- 4. APPLY ROBUST CANVAS_MEMBERS POLICIES (Essential for upsert/join)
CREATE POLICY "Select_Members" ON canvas_members FOR SELECT USING ( public.is_canvas_member(canvas_id) );
CREATE POLICY "Insert_Members" ON canvas_members FOR INSERT WITH CHECK ( (user_id = auth.uid()) OR public.is_canvas_member(canvas_id) );
CREATE POLICY "Update_Members" ON canvas_members FOR UPDATE USING ( (user_id = auth.uid()) OR public.is_canvas_member(canvas_id) );
CREATE POLICY "Delete_Members" ON canvas_members FOR DELETE USING ( (user_id = auth.uid()) OR public.is_canvas_owner(canvas_id) );
...
CREATE POLICY "Select_Items" ON canvas_items FOR SELECT USING ( public.is_canvas_member(canvas_id) );
CREATE POLICY "Insert_Items" ON canvas_items FOR INSERT WITH CHECK ( public.is_canvas_member(canvas_id) );
CREATE POLICY "Update_Items" ON canvas_items FOR UPDATE USING ( public.is_canvas_member(canvas_id) );
CREATE POLICY "Delete_Items" ON canvas_items FOR DELETE USING ( author_id = auth.uid() OR public.is_canvas_owner(canvas_id) );

-- 6. SYNC EXISTING DATA (Optional: ensures creators are members with 'owner' role)
