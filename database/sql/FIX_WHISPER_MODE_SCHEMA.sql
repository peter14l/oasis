...
-- Note: In a real Supabase environment, you might use pg_cron or a Edge Function.
-- This part just ensures we have the index for it.
CREATE INDEX IF NOT EXISTS idx_messages_expires_at_cleanup 
ON public.messages(expires_at) 
WHERE expires_at IS NOT NULL;

-- 5. Logic to automatically delete expired messages when accessed 
-- (optional backup for the cron job)
