-- Migration: Fix send_message_v3 notifications metadata
-- Purpose: Populate metadata JSONB in notifications table with E2EE metadata
--          (encrypted_keys, iv, signal_message_type, signal_sender_content, pq_aura_header, pq_aura_payload)
--          so client-side notification decryption services can decrypt push/local notifications.

-- 1. Ensure metadata column exists on notifications table
DO $$ 
BEGIN
...
-- 3. Recreate send_message_v3 with full returned fields and notifications metadata
...
BEGIN
    ...
    -- 3. Trigger notifications with E2EE metadata
    INSERT INTO notifications (user_id, type, actor_id, conversation_id, message_id, title, content, metadata)
    SELECT
        cp.user_id,
        'dm',
        v_sender_id,
        ...
        END,
        jsonb_build_object(
            'encrypted_keys', p_encrypted_keys,
            'iv', p_iv,
            'signal_message_type', p_signal_message_type,
            'signal_sender_content', p_signal_sender_content,
            'pq_aura_header', p_pq_aura_header,
            'pq_aura_payload', p_pq_aura_payload,
    ...
    -- 5. Return the created message as JSON (including voice duration and encryption metadata)
    SELECT json_build_object(
        ...
        'encrypted_keys', m.encrypted_keys,
        'iv', m.iv,
        'share_data', m.share_data,
        'location_data', m.location_data,
        'media_view_mode', m.media_view_mode,
        'signal_message_type', m.signal_message_type,
        'signal_sender_content', m.signal_sender_content,
