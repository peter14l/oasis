...
class _CircleJoinScreenState extends State<CircleJoinScreen> {
  ...
  Future<void> _loadCircle() async {
    try {
      final circle = await context.read<CircleProvider>().getCircle(
        widget.circleId,
      );
      setState(() {
        _circle = circle;
  ...
  Future<void> _join() async {
    ...
    try {
      await context.read<CircleProvider>().joinCircle(widget.circleId, userId);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Stack(
        children: [
          Positioned.fill(
            child: FluidMeshBackground(streakCount: _circle!.streakCount),
          ),
          SafeArea(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(_circle!.emoji, style: const TextStyle(fontSize: 80)),
