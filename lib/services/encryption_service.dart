...
import 'package:oasis_v2/services/key_management_service.dart';

export 'package:oasis_v2/services/key_management_service.dart' show EncryptionStatus;
...
class EncryptionService {
  ...
  Future<EncryptionStatus> init() async {
    ...
    try {
      ...
      if (legacyPrivate != null && legacyPublic != null) {
        ...
        final backupKey = _keyManager.deriveBackupKey(userId);
      ...
      // 3. Server Check & Auto-Restore
      final response = await _supabase.from('profiles').select('encrypted_private_key, public_key').eq('id', userId).maybeSingle();
  ...
  Future<bool> setupEncryption() async {
    try {
      ...
      await _supabase.from('profiles').update({'public_key': publicKeyPem, 'encrypted_private_key': encryptedPrivateKey}).eq('id', userId);
  ...
  Future<bool> backupSignalIdentity(String identityKeyPairBase64, int registrationId) async {
    try {
      final userId = _supabase.auth.currentUser?.id;
      if (userId == null) return false;
      final encrypted = _keyManager.encryptWithKey(jsonEncode({'identityKeyPair': identityKeyPairBase64, 'registrationId': registrationId}), _keyManager.deriveBackupKey(userId));
  ...
  Future<Map<String, dynamic>?> restoreSignalIdentity() async {
    try {
      ...
      final decrypted = _keyManager.decryptWithKey(response['encrypted_signal_identity'] as String, _keyManager.deriveBackupKey(userId));
