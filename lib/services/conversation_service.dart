...
class ConversationService {
  ...
  Future<Conversation> getConversationDetails(String conversationId) async {
    try {
      ...
      final conversations = await getConversations(userId: userId, limit: 100);
      final index = conversations.indexWhere((c) => c.id == conversationId);
      
      if (index != -1) return conversations[index];

      throw Exception('Conversation not found: $conversationId');
    } catch (e) {
      debugPrint('[ConversationService] Error fetching details: $e');
      rethrow;
    }
  }
