...
class MessageOperationsService {
    ...
    try {
      ...
      if (response != null) {
        final mediaUrls = [
          ...
          response['file_url'],
          response['voice_url'],
        ].where((url) => url != null && url.toString().isNotEmpty).cast<String>().toList();

        await _supabase.from(SupabaseConfig.messagesTable).delete().eq('id', messageId);
  ...
  Future<void> clearConversationMessages(String conversationId) async {
    try {
      await _supabase.from(SupabaseConfig.messagesTable).delete().eq('conversation_id', conversationId);
      await _supabase.from(SupabaseConfig.conversationsTable).update({'last_message_id': null, 'last_message_at': null}).eq('id', conversationId);
    } catch (e) {
  ...
  RealtimeChannel subscribeToTypingStatus({
    ...
    channel.onPostgresChanges(
      ...
      callback: (payload) {
        ...
        if (data.isNotEmpty) {
          onTypingUpdate(data['user_id'] as String, data['is_typing'] as bool? ?? false);
        }
      },
    ).subscribe((status, [error]) {
      if (status == RealtimeSubscribeStatus.channelError) {
        debugPrint('MessageOperationsService: subscribeToTypingStatus error: $error');
      }
    });
    return channel;
  }
  ...
  Future<void> markConversationAsRead(String conversationId, String userId) async {
    try {
      await _supabase.rpc(SupabaseConfig.resetUnreadCountFn, params: {
  ...
  Future<void> markMessagesAsRead(String conversationId, List<String> messageIds, String userId) async {
    try {
      if (messageIds.isEmpty) return;
      
      // Upsert read receipts for each message
      final List<Map<String, dynamic>> receipts = messageIds.map((id) => {
        'message_id': id,
        'user_id': userId,
        'read_at': DateTime.now().toIso8601String(),
      }).toList();

      await _supabase.from(SupabaseConfig.messageReadReceiptsTable).upsert(receipts, onConflict: 'message_id,user_id');
  ...
  Future<void> incrementMediaViewCount(String messageId) async {
    try {
      ...
      await _supabase.from(SupabaseConfig.messageMediaViewsTable).upsert({
        ...
        'user_id': userId,
        'view_count': 1, // This is a simplified version, usually you'd increment it
  ...
  RealtimeChannel subscribeToReadReceipts({
    ...
    channel.onPostgresChanges(
      ...
      if (status == RealtimeSubscribeStatus.channelError) {
        debugPrint('MessageOperationsService: subscribeToReadReceipts error: $error');
      }
    });
    return channel;
  }
  ...
  RealtimeChannel subscribeToConversationDetails({
    ...
    channel.onPostgresChanges(
      ...
      if (status == RealtimeSubscribeStatus.channelError) {
        debugPrint('MessageOperationsService: subscribeToConversationDetails error: $error');
      }
    });
    return channel;
  }
