...
- **Global scope**: `F:/oasis/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/oasis/.opencode/notes/`)
...
2. Glob `F:/oasis/.opencode/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `F:/oasis/.opencode/notes/` — works in any directory
