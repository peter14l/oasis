...
- **Global scope**: `D:/oasis/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/oasis/.opencode/notes/`)
...
2. Glob `D:/oasis/.opencode/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `D:/oasis/.opencode/notes/` — works in any directory
