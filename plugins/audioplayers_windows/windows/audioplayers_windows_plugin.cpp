...
UINT GetAudioplayersDispatchMessage() {
  static UINT msg = RegisterWindowMessageA("AudioplayersWindowsPluginDispatch");
  return msg;
}
...
class AudioplayersWindowsPlugin : public Plugin {
 public:
  ...
  AudioplayersWindowsPlugin(HWND hwnd);
 ...
 private:
  ...
  HWND hwnd_;
  EventStreamHandler<>::Dispatcher dispatcher_;
...
void AudioplayersWindowsPlugin::RegisterWithRegistrar(
  ...
  auto plugin = std::make_unique<AudioplayersWindowsPlugin>(hwnd);
  ...
  if (view) {
    registrar->RegisterTopLevelWindowProcDelegate(
        [plugin_ptr = plugin.get()](HWND hwnd, UINT message, WPARAM wparam,
          ...
          if (message == GetAudioplayersDispatchMessage()) {
  ...
  globalEvents = std::make_unique<EventStreamHandler<>>(plugin->dispatcher_);
...
AudioplayersWindowsPlugin::AudioplayersWindowsPlugin(HWND hwnd) : hwnd_(hwnd) {
  dispatcher_ = [this](std::function<void()> task) {
    auto* task_ptr = new std::function<void()>(std::move(task));
    if (!PostMessage(hwnd_, GetAudioplayersDispatchMessage(),
...
void AudioplayersWindowsPlugin::CreatePlayer(std::string playerId) {
  ...
  auto eventHandler = new EventStreamHandler<>(dispatcher_);
  auto _obj_stm_handle =
