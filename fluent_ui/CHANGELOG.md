...
- feat: Modernize UI to align with WinUI 3 (Windows 11) guidelines:
  - **Typography**: Added support for **Segoe UI Variable** as the default font family on Windows, with automatic fallback to Segoe UI.
  - **Corner Radius**: Updated default `BorderRadius` to **8px** for major surfaces (`Card`, `Flyout`, `ContentDialog`, `TextBox`) for a more modern look.
  - **Mica Alt**: Introduced support for **Mica Alt** material in `FluentThemeData` and `Mica` widget for secondary app surfaces.
  - **Shadows**: Implemented a standardized **FluentShadows** system (`elevation2`, `elevation8`, `elevation32`, `elevation64`) and applied it across buttons, cards, flyouts, and dialogs.
  - **Dynamic Materials**: `Mica` and `Acrylic` now automatically respond to window focus, switching to a solid background color when the application is unfocused to reduce distraction.
  - **TextBox Visuals**: Enhanced `TextBox` focus indicator with a **2px accent-colored bottom border** and 1px unfocused border, matching modern WinUI 3 specs.
  - **Animations**: Updated default animation curves to `easeInOutCubic` and introduced `entranceCurve` and `exitCurve` for smoother, more fluid "Fluid" motion.
- feat: Added **`PipsPager`** control for modern page navigation.
